...
"""
import argparse # Used to read command-line arguments (python eval.py --limit 10 --pdfs-only)
import json
import time # Used to measure how long each document takes
from pathlib import Path # Used to work with files and folders
...
def run_eval(limit: int | None, pdfs_only: bool, out_path: str, lender: str | None = None) -> None:
    ...
    gt: dict = json.loads(GROUND_TRUTH.read_text(encoding="utf-8")) # Load synthetic ground-truth labels generated by generate.py

    entries = list(gt.items())
    if pdfs_only: # Optional filter: evaluate only PDF documents
        entries = [(k, v) for k, v in entries if k.endswith(".pdf")]
    if lender: # Optional filter: evaluate documents from a specific lender
        entries = [(k, v) for k, v in entries if k.startswith(lender)]
    if limit: # Optional filter: evaluate only the first N documents
    ...
    # accumulators
    status_correct = 0 # Number of documents whose final status matched ground truth
    needs_review_count = 0 # Count of documents routed to manual review
    field_hits: dict[str, int]   = {}
    field_total: dict[str, int]  = {}
    confusion = {"TP": 0, "TN": 0, "FP": 0, "FN": 0}   # pos = CHANGES_REQUESTED
    per_doc: list[dict] = []  # Detailed result for every evaluated document
    skipped = 0
    ...
    for i, (filename, entry) in enumerate(entries, 1):
        ...
        # Expected labels from ground truth
        ...
        try:
            ...
            print(f"  [{i}/{total}] ERROR {filename}: {e}") # Record failures without stopping the entire evaluation
        ...
        if got_status == "NEEDS_REVIEW":  # Track how often documents are sent for manual review
            needs_review_count += 1

        if status_ok:   # Track overall status accuracy
            status_correct += 1
        # confusion matrix (positive class = CHANGES_REQUESTED)
        # True Positive: Bad document correctly rejected.
        if expected_status == "CHANGES_REQUESTED" and got_status == "CHANGES_REQUESTED":
            confusion["TP"] += 1
        # True Negative: Good document correctly approved.
        elif expected_status == "APPROVED" and got_status == "APPROVED":
            confusion["TN"] += 1
        # False Positive: Good document incorrectly rejected.
        elif expected_status == "APPROVED" and got_status == "CHANGES_REQUESTED":
            confusion["FP"] += 1
        # False Negative: Bad document not rejected when it should have been.
        else:
            confusion["FN"] += 1
        ...
        for field in FIELDS:
            ...
            if not expected_val: # Skip fields not expected for this document type
                continue
            field_total[field] = field_total.get(field, 0) + 1
            got_raw = (extracted or {}).get(field)
            hit = _fields_match(field, got_raw, expected_val) # Determine whether this field was extracted correctly
            if hit: 
    ...
    all_hits = 0
    all_total = 0
    for field in sorted(field_total): # Report extraction accuracy for each field individually
